    ...
    <tbody>
        @foreach ($listaPrecios as $listaPrecio)
            ...
            <tr id={{ $listaPrecio->cuit }}>
                ...
                <td class="col-2 fechaUpdate">
                    {{ $listaPrecio->modificado }}
                </td>

                <td class="col-1" style="vertical-align: middle; text-align:center;">
                    @if($listaPrecio->prods > 0)
                        <a href="{{ route('administracion.listaprecios.editar', ['razon_social' => $listaPrecio->razon_social]) }}" class="btn btn-sm btn-link" 
                    ...
                    @else
                        <a href="{{ route('administracion.listaprecios.editar', ['razon_social' => $listaPrecio->razon_social]) }}" class="btn btn-sm btn-link text-success" 
